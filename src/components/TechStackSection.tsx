...
export function TechStackSection() {
  const stacks = [
    ...
    {
      title: "Backend",
      items: ["Node.js", "ExpressJs", "MongoDB"],
    ...
    <section className='bg-gradient-to-b from-[#F7F9FC] via-purple-50/20 to-white py-24 relative overflow-hidden'>
      ...
      <div className='max-w-7xl mx-auto px-6 relative'>
        <div className='text-center mb-16 animate-fade-in-up'>
          <h2 className='text-[#111827] mb-4 bg-gradient-to-r from-blue-600 via-purple-600 to-orange-600 bg-clip-text'>
            Tech Stack
          </h2>
        ...
        <div className='grid md:grid-cols-3 gap-8'>
          {stacks.map((stack, index) => (
            <div
              ...
              <div
                className={`absolute inset-0 bg-gradient-to-br ${stack.gradient} opacity-0 group-hover:opacity-5 transition-opacity duration-500`}></div>

              {/* Animated gradient border */}
              <div className='absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500'>
                <div
                  className={`absolute inset-[-2px] bg-gradient-to-br ${stack.gradient} rounded-3xl -z-10 animate-shimmer bg-[length:200%_100%]`}></div>
              </div>

              <div className='relative z-10'>
                <div className='text-6xl mb-6 text-center transform group-hover:scale-125 group-hover:rotate-12 transition-all duration-500'>
                  {stack.icon}
                </div>
                <h3
                  className={`text-center bg-gradient-to-r ${stack.gradient} bg-clip-text text-transparent group-hover:scale-110 transition-transform`}>
                  {stack.title}
                </h3>
              </div>
